# 19 - Spring 事务管理：理解 `@Transactional` 什么时候生效
...
-   **`propagation`**: 定义事务如何传播。`REQUIRED` 表示如果当前没有事务就新建一个，如果有就加入。
...
- `@Transactional` 优先放在 Service 层，而不是 Controller 层
- 写操作建议显式加 `rollbackFor = Exception.class`
- 查询场景可以考虑 `readOnly = true`
- 不要把事务包住长时间的外部 HTTP / RPC 调用
